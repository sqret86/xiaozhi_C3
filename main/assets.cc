...
// LVGL and emote display support (optional)
#ifdef HAVE_LVGL
...
bool Assets::Apply() {
    ...
    if (cJSON_IsString(srmodels)) {
        ...
        if (GetAssetData(srmodels_file, ptr, size)) {
            ...
            if (models_list_ != nullptr) {
                // Note: AudioService not available in current version
                // Models will be loaded but not set to audio service
                ESP_LOGI(TAG, "SR models loaded successfully");
            } else {
