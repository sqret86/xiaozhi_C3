...
Http* Ota::SetupHttp() {
    ...
    // Accept-Language: 优先使用运行时检测/保存的语言代码，其次退回编译时语言
    {
        Settings lang_settings("language", false);
        std::string lang_code = lang_settings.GetString("code");
        if (lang_code.empty()) {
            lang_code = Lang::CODE;
        }
        http->SetHeader("Accept-Language", lang_code.c_str());
    }
...
bool Ota::Upgrade(const std::string& firmware_url) {
    ...
    esp_restart();
    return true; // This line will never be reached, but needed for return type
}
